...
UI_OVS = [x.strip() for x in os.environ.get("UI_OVS", "obum,obuv,obub,obuo,obuw,obus").split(",") if x.strip()]
...
@app.get("/ui/ov", response_class=HTMLResponse)
def ui_ov_get(request: Request, next: str = "/ui/groupmap"):
    ...
    options = "\n".join(
        f'<option value="{ov}" {"selected" if ov == current else ""}>{ov}</option>'
        for ov in UI_OVS
    )
...
<body>
  <h2>OV auswählen</h2>
  <form method="post" action="/ui/ov">
    <input type="hidden" name="next" value="{next}">
    <label>OV</label><br>
    <select name="ov">{options}</select>
    <button type="submit">Weiter</button>
  </form>
  <div class="hint">Wird als Standard im Browser gespeichert.</div>
</body>
</html>
"""
    return html
...
async def ui_ov_post(request: Request):
    ...
    # Default: immer auf groupmap mit ov
    url = f"/ui/groupmap?ov={ov}"
    # falls später andere Seiten kommen, könnt ihr nxt nutzen – für jetzt simpel halten.
